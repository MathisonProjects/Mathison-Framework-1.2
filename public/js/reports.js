      ...
      dataType: 'json',
  	  success: function(data) {
          ...
          $.each(data, function(index, element) {
            ...
            $('#filterArea').html($('#filterArea').html() + '<div class="form-group"><label for="filter['+index+']">' + element + '</label><br /><div class="checkbox"><label><input type="checkbox" name="filter['+index+'][filtered]"> Filter</label></div><select class="form-control" name="filter['+index+'][operator]"><option></option><option value="contains">Contains</option><option value="==">Equals</option><option value="!=">Does Not Equal</option><option value=">">Greater Than</option><option value="<">Less Than</option><option value=">=">Greater Than or Equal To</option><option value="<=">Less Than or Equal To</option></select><input type="text" class="form-control" name="filter['+index+'][comparison]" /></div>');
